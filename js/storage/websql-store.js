var WebSqlStore = function(successCallback, errorCallback) {
    ...
    //My method
    this.addSamplePerson = function(){
    	this.db.transaction(
                function(tx) {
			    	var p = {"firstName": "Ryan", "lastName": "Howard", "title":"Vice President, North East", "managerId": 0, "city":"New York, NY", "cellPhone":"212-999-8888", "officePhone":"212-999-8887", "email":"ryan@dundermifflin.com"};
			    	var sql = "INSERT INTO employee (firstName, lastName, managerId, title, city, officePhone, cellPhone, email) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
			    	tx.executeSql(sql, [p.firstName, p.lastName, p.managerId, p.title, p.city, p.officePhone, p.cellPhone, p.email],
			                function() {
			                    console.log('INSERT sample success');
			                },
			                function(tx, error) {
			                    alert('INSERT sample error: ' + error.message);
			                });
                },
                function(error) {
